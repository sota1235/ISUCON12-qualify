...
async function billingReportByCompetition(
  ...
  try {
    // スコアを登録した参加者のIDを取得する
    const [scoredPlayerIds] = await adminDB.query<(string & RowDataPacket)[]>(
...
app.get(
  ...
  wrap(async (req: Request, res: Response) => {
    try {
      ...
      const [competitionScores] = await adminDB.query<(CompetitionScore & RowDataPacket)[]>(
          'SELECT t1.id    as competition_id,\n' +
          '       t2.score as score\n' +
